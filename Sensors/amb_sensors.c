...
volatile uint8_t Rawdata[11][8];		//This is an array for 8 sensors + Gyro temp, each one having up to 8 (4*16bit) bytes of data per sample
...
void Fill_Sample_Buffers(void) {
	for(uint8_t m=0;m<2;m++) {		//Init the two sparkfun sensor buffers
		for(uint8_t n=0;n<3;n++) {
			Add_To_Buffer(*(uint16_t*)&(Rawdata[4+4*m][2*n]),		&(sfe_sensor_buffers[m].accel[n]));
			Add_To_Buffer(Flipedbytes(*(uint16_t*)&(Rawdata[5+4*m][2*n])),	&(sfe_sensor_buffers[m].magno[n]));
			Add_To_Buffer(Flipedbytes(*(uint16_t*)&(Rawdata[6+4*m][2*n+2])),&(sfe_sensor_buffers[m].gyro[n]));//+2 as we skip the temperature
		}
		Add_To_Buffer(Flipedbytes(*(uint16_t*)&(Rawdata[6+4*m][0])),&(sfe_sensor_buffers[m].temp));//the temperature from the itg3200
	}
	//Add the LSM330 temperature sensor (this is only 8 bit output)	- other LSM330 sensor data is added directly from callbacks
	Add_To_Buffer((uint16_t)Rawdata[7][0],&(forehead_buffer.temp));
}
